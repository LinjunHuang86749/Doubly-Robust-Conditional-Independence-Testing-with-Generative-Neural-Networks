...
bn = as.numeric(command.args[2])##1,2. 1: uniform data, 2:true data
...
for (rr in ((dt-1)*ddt+1):(dt*ddt)) {
  ...
  if(bn == 2){
    ...
    Y <- cos(beta_Y%*% t(Z) + error_Y) |> as.vector()
    h = 15
    temp_result = CI.test(X, Y, Z, h, bn = bn) * n
  }
  if(bn == 1){
    # cat(paste("runing bn is 1. \n"))
    Z <- runif(dz*n) |> matrix(ncol = dz) 
    X <- runif(n) |> as.vector()
    Y <- runif(n) |> as.vector()
    h = 15
    temp_result = CI.test(X, Y, Z, h, bn = bn) * n
...
write.csv(resultraw,paste0("./result/cit_size_dz",dz,"_bn",bn,"_dt",dt,"_v1.csv"),row.names = F)
...
dz_set =c(50,100,150,200,250)
dt_set = seq(1,25,1)
data1=c()



for (dz in c(50,100,150,200,250)) {
  data1temp = c(n,dz)
  for (dt in dt_set) {
    temp=read.csv(file=paste0("./result/cai/cit_size_dz",dz,"_bn",bn,"_dt",dt,"_v1.csv"),header = T)
...
for (i in c(1,2,3,4,5)) {
  ...
  r95 = sum(data1[i,4:length(data1[1,])]>rejcc[i])/reptime
  r90 = sum(data1[i,4:length(data1[1,])]>rejcc10[i])/reptime
