...
def run_task(ppvalue=0.0,n_points = 200, n_xy_points = 200,n_zy_points = 200, device=device, dim=2,  kernel_x = 'gaussian', kernel_yx = 'gaussian', kernel_z= 'gaussian', kernel_yz= 'gaussian', kernel_yx_args= {'sigma2': 1.0, 'n': 3},
             ...
             *, task='get_xzy_randn_nl', ground_truth='H0', measure = 'kci_xsplit', xzy_holdout_sampling='joint',  rbpt_c=0.1, rbpt_gamma=0.01, rbpt_seed=1,
...
#p_val_mx = np.zeros((1000, 5)) # (number of p_values, number of different n)
index = 0
for n in [1000,1600,2000]:
    if n ==1000:
      for fd in [3,4,5]:
        testsset = [500,357,126,56,31]
        pppaaa=0.0
        ...
        emp_rej_rate = torch.mean((p_val_list_temp < 0.05).float()).item()
        print("finish n = {}, emp rej rate for 5% is {}".format(n, emp_rej_rate))
        p_val_mx_temp[:, 0] = p_val_list_temp.reshape(-1).cpu().numpy()
        np.savetxt('./p_val_mx_n_'+ str(n) +'_fd_' + str(fd)+'_ppvalue_' + str(pppaaa)  +'.csv', p_val_mx_temp, delimiter=",")
        
    if n ==1600:
      for fd in [1,2,3,4,5]:
        testsset = [800,555,179,75,40]
        pppaaa=0.0
        n_test = testsset[(fd-1)]
        n_train = n-n_test  
        p_val_mx_temp = np.zeros((1000, 1)) # (number of p_values, number of different n)
