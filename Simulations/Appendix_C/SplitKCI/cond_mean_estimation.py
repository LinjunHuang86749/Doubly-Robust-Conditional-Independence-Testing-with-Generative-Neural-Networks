...
def compute_single_loo_error(K_yy, K_QQ, reg, cpu_solver=False, cpu_dtype=np.longdouble):
    n = K_yy.shape[0]
    if cpu_solver:
...
def compute_loo_errors(K_yy, K_QQ, lambda_values=None, verbose=False, cpu_solver=False, cpu_dtype=np.longdouble):
...
def leave_one_out_regressors_single_kernel(y, K_zz, kernel_y, lambda_values=None, param_dict=None, default_y_args=None,
                                           verbose=True, cpu_solver=False, cpu_dtype=np.longdouble):
...
def leave_one_out_regressors(y, K_zz, kernel_y, lambda_values=None, param_dict=None, default_y_args=None, verbose=True,
                             cpu_solver=False, cpu_dtype=np.longdouble):
    if isinstance(kernel_y, list):
        ...
        return leave_one_out_regressors_single_kernel(y, K_zz, kernel_y, lambda_values, param_dict, default_y_args,
                                                      verbose, cpu_solver=cpu_solver, cpu_dtype=np.longdouble)
...
def get_yz_regressors_half_split(y, z, kernel_y, kernel_z, kernel_y_args, kernel_z_args, param_dict=None,
                                 lambda_values=None, verbose=True, cpu_solver=False, cpu_dtype=np.longdouble):
    n_splits = 2
    n = y.shape[0]
    if n % n_splits != 0:
...
class KernelRidgeRegression(torch.nn.Module):
    ...
    def fit(self, x, y, verbose=False, cpu_solver=True, cpu_dtype=np.longdouble,
